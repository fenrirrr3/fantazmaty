...
def last_activity(request):
    ...
    from django.db.models import OuterRef, Subquery, Q, F, Value, CharField
    ...
    if mode == 'login':
        ...
        history = WorkflowEvent.objects.filter(actor_id=OuterRef('user_id'), personal_work=True).order_by('-created_at', '-pk')
    ...
    if days:
        people = people.filter(Q(last_activity_at__lte=timezone.now()-timedelta(days=days)) | Q(last_activity_at__isnull=True))
